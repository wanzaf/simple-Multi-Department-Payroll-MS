...
async function loadEmployees() {
    ...
    tbody.innerHTML = `<tr><td colspan="10"><div class="empty-state"><p>Loading…</p></div></td></tr>`;
...
function renderPage() {
    ...
    if (perPage === 0) {
        pageEmps = allEmployees;
        currentPage = 1;
    } else {
        lastPage = Math.ceil(total / perPage);
        currentPage = Math.min(currentPage, lastPage);
        const start = (currentPage - 1) * perPage;
        pageEmps = allEmployees.slice(start, start + perPage);
    }

    const globalOffset = perPage === 0 ? 0 : (currentPage - 1) * perPage;

    tbody.innerHTML = pageEmps
        .map(
            (emp, i) => `
...
function renderPagination(total, perPage, lastPage) {
    ...
    info.textContent = `Showing ${start}–${end} of ${total} employees`;
